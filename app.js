...
function toggleLanguage() {
  ...
  localStorage.setItem('primeLang', currentLang);
...
function highlightActiveNav() {
  ...
  links.forEach(link => {
    ...
    if (href && path.includes(href.split('#')[0]) && href !== 'index.html') {
      link.classList.add('active');
    } else if ((path.endsWith('/') || path.endsWith('index.html')) && (href === 'index.html' || href === '#home')) {
      link.classList.add('active');
    }
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const savedLang = localStorage.getItem('primeLang');
...
function renderProjects(filterCategory = 'all') {
  ...
  grid.innerHTML = filtered.map(proj => {
    ...
    const isUrl = proj.heroGrad && (proj.heroGrad.startsWith('http') || proj.heroGrad.startsWith('url'));
    const bgStyle = isUrl ? `background-image: url('${proj.heroGrad.replace(/^url\(['"]?/, '').replace(/['"]?\)$/, '')}'); background-size: cover; background-position: center;` : `background: ${proj.heroGrad};`;

    return `
      <div class="project-card">
        <div class="project-hero-sample" style="${bgStyle}">
...
function openProjectModal(projectId) {
  ...
  body.innerHTML = `
    <a href="${isUrl ? proj.heroGrad : '#'}" target="_blank" style="display: block; cursor: ${isUrl ? 'zoom-in' : 'default'};">
      <div style="height: 250px; border-radius: var(--radius-md); ${bgStyle} margin-bottom: 1.25rem; border: 1px solid var(--border-gold);"></div>
    </a>
    ...
    <div style="background: #0a0d14; border: 1px solid var(--border-gold); border-radius: 8px; padding: 1rem; margin-bottom: 1.25rem;">
      ...
      <p style="font-size: 0.85rem; color: var(--text-main);">${scope || 'توريد وتركيب الرخام'}</p>
    </div>
